...
if __name__ == "__main__":
    ...
    args.add_argument("--local_files_only", action="store_true", help="enable it to use cached files without requesting from the hub")
    ...
    #use gpt to generate the substitute
    ...
    remove_res_folder_path = "../data_processing/assets/3_editing_pair_construction/replace/target_imgs"
    output_folder = "../data_processing/assets/3_editing_pair_construction/replace/json_file"
    ...
    json_files_to_process = [json_file for json_file in json_files if start_range <= extract_number(json_file) < end_range]

    if not json_files_to_process:
        print("没有要处理的文件。请检查 range 参数。")
    else:
        for json_file in json_files_to_process:
            ...
            try:
                ...
                for index, annotation in enumerate(annotations):
                    ...
                    replace_json = call_gpt(global_caption, class_name, detailed_caption)
                    if replace_json is None or replace_json["flag"] == 0:
                        continue

                    replace_class = replace_json["replace_class_name"]
                    replace_class_caption = replace_json["replace_detailed_caption"]

                    res1, res2 = controller.infer(
                        input_image=input_image,
                        text_guided_prompt=replace_class,
                        text_guided_negative_prompt="",
                        shape_guided_prompt="",
                        shape_guided_negative_prompt="",
                        fitting_degree=1,
                    ...
                    class_name = preprocess_class_name(class_name)
                    replace_class = preprocess_class_name(replace_class)
                    mask_filename = f"{json_file.replace('.json', '')}_{index + 1}_{class_name}_to_{replace_class}.png"
                    ...
                    replace_instruction = replace_class.replace('_', ' ')
                    replace_instruction1 = [
                        "change the " + class_instruction + " to " + replace_instruction,
                        "replace the " + class_instruction + " with " + replace_instruction
                    ]

                    replace_instruction2 = [
                        "change the " + replace_instruction + " to " + class_instruction,
                        "replace the " + replace_instruction + " with " + class_instruction
                    ]

                    res_json[str(count)] = {
                        "target_img_path": remove_res_path,
                        "origin_img_path": image_path,
                        "instruction": replace_instruction1,
                        "task_type": "replace",
                        "detailed_caption": annotation["detailed_caption"],
                        "img_caption": global_caption,
                        "class_origin": class_instruction,
                        "class_target": replace_instruction,
                        "detailed_caption_origin": detailed_caption,
                        "detailed_caption_target": replace_class_caption,
                        "bbox": bbox
                    }
                    count += 1  
                    ...
                    res_json[str(count)] = {
                        ...
                        "instruction": replace_instruction2,
                        "task_type": "replace",
                        "detailed_caption": annotation["detailed_caption"],
                        "img_caption": global_caption,
                        "class_origin": replace_instruction,
                        "class_target": class_instruction,
                        "detailed_caption_origin": replace_class_caption,
                        "detailed_caption_target": detailed_caption,
                        "bbox": bbox
                    }
                    count += 1  
                ...
                with open(output_json_path, 'w') as json_out:
                    json.dump(res_json, json_out, indent=4)
