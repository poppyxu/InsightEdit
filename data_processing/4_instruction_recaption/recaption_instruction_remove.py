...
system_prompt = '''
            ...
            **任务描述**：
            这是一个图像移除的任务，你需要根据给定的信息来生成一条图像编辑的指令文本。您将接收到两张张图像，一张是移除前的图像，一张是移除后的图像，和一条简单的移除内容的指令。您的目标是根据这些信息将原始指令转写成带有推理或理解能力的指令。
...
system_prompt_en = '''
            ...
            **Task Description**:
            This is an image removal task. You need to generate an image editing instruction based on the given information. You will receive two images: one is the image before removal, and the other is the image after removal, along with a simple instruction for the removal. Your goal is to rewrite the original instruction into one that demonstrates reasoning or understanding based on these images.

            **Guidelines**:
            1. All content in the JSON file must be in English.
            2. This task involves removing the "class_origin" from the current image. Please generate an instruction that avoids simple phrasing like "remove object1", and instead reflects reasoning and understanding.
            ...
            5. Try to avoid mentioning the specific name of the original object. Use abstract descriptions like "the object on the left", "the object next to it", etc.
            6. Preferably, frame the instruction as a question. Identify the object being removed by referring to the content of the image, and then modify it.
...
if __name__ == "__main__":
    source_directory = '../InsightEdit/data_processing/assets/3_editing_pair_construction/remove_add/json_file'
    destination_directory = "../InsightEdit/data_processing/assets/4_instruction_recaption/remove_add"
